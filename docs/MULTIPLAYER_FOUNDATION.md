...
- Verification on 2026-09-24: The first two-PC Tailscale test found that remote bodies stayed at spawn and LMB grabs did not work. The repaired Windows development build and Unity 6000.5.11f1 compilation succeeded. Editor Host spawned one owner and eight bodies; moving its KCC motor moved the NetworkTransform target and visual. Separate localhost host plus two client processes observed owner position changes of 4 m on all peers and a 45-degree camera-yaw change on remote visuals. A development probe invoked the same raycast/grab/hold/release RPC path as LMB: the server moved a 5 kg box, observers received its motion, a second holder was refused, disconnect released it, and another participant then grabbed and released it. This does not replace a two-human Tailscale retest of movement feel, real LMB input, contention, and release/reconnect.
