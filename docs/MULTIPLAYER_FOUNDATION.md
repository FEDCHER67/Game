...
- The server spawns one `NetworkPlayer` for each connection after start scenes load and assigns one of four free spawn slots. The owning client runs the unchanged KCC input, camera, and network-specific grab input. Other instances disable their local KCC motor, input, camera, and AudioListener. FishNet NetworkTransform is attached to the KCC motor child that actually moves, so owner position and capsule yaw reach the server and observers. The remote visual follows that child; a separate small unreliable RPC sends camera yaw to the body and pitch to its head without rotating the collision capsule.
- The server alone simulates eight `NetworkPhysicsBody` Rigidbody instances (5, 20, 50, and 100 kg). Clients keep the bodies kinematic and observe their FishNet NetworkTransform state. The server verifies an LMB grab ray, connection ownership, distance from the moving KCC motor, allowed body, and free holder slot. Exactly one holder is recorded per body; only server FixedUpdate applies hold forces. Release and disconnect clear that holder.
- Local `PhysicsGrabber`, network input `NetworkGrabber`, and server `NetworkPhysicsBody` reference one `Assets/OnlyVolunteers/Player/GrabPhysicsProfile.asset`. `GrabPhysicsSolver` computes the same single-point spring/damping force and applies the same speed caps for local and server bodies. The solver has no FishNet dependency; RPC timing, holder state, and stale-target timeout stay in the network layer. `-ov-smoke-move` and `-ov-smoke-grab` remain Editor/development-build regression probes and do nothing without those flags.
...
- Verification on 2026-09-24: The first two-PC Tailscale test found stationary remote bodies and unusable grabs; commit `3fa69c2d972b7aa0aca70bc5f574769941dd22a7` repaired those faults, and two humans subsequently confirmed host/client connection, mutual movement visibility, local KCC movement, and shared physics interaction over Tailscale. The later shared-solver cleanup compiled and passed an Editor local grab check, Editor Host check, and localhost Host + two-client movement/grab regression probes. Fixed inputs produced exactly the same forces before and after extraction. A new two-human feel test of the cleanup build remains pending.
